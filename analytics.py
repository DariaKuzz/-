...
from base import create_db_connection, DB_FILE
...
def get_flight_prices_from_db(origin, destination, start_date, end_date, conn):
    ...
    try:
        cursor = conn.cursor()
        query = f"""
        SELECT departure_date, MIN(price_rub) as min_price
        FROM flights
        WHERE origin_iata = '{origin}' 
        AND destination_iata = '{destination}' 
        AND departure_date BETWEEN '{start_date}' 
        AND '{end_date}'
...
def statistical_analysis(df):
    ...
    try:
        decomposition = sm.tsa.seasonal_decompose(df.set_index('departure_date')['min_price'], model='additive', period=30)
...
def visualize_price_trends(df, origin, destination, start_date, end_date):
    ...
    plt.plot(df['departure_date'], df['min_price'], marker='o', linestyle='-')
...
def train_and_predict(df, origin, destination):
    ...
    # Подготовка данных
    df['days_since_start'] = (df['departure_date'] - df['departure_date'].min()).dt.days
    ...
    # Прогноз на будущее
    last_date = df['departure_date'].max()
    future_dates = [last_date + timedelta(days=i) for i in range(1, 31)]
    future_df = pd.DataFrame({'departure_date': future_dates})
    future_df['days_since_start'] = (future_df['departure_date'] - df['departure_date'].min()).dt.days
    ...
    print(future_df[['departure_date', 'predicted_price']])

    # Визуализация прогноза
    plt.figure(figsize=(12, 6))
    plt.plot(df['departure_date'], df['min_price'], marker='o', linestyle='-', label='Фактические цены')
    plt.plot(future_df['departure_date'], future_df['predicted_price'], marker='x', linestyle='--', label='Прогноз')
    plt.xlabel("Дата вылета")
    plt.ylabel("Цена (руб)")
